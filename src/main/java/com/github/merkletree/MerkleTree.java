...
/**
 * The basic idea is that of a binary hash tree with leaf nodes representing
 * first level hashes and the parent node data being a concatenation or hash of
 * hashes of its child nodes thus the top-level hash represents a union of all
 * hashes in the tree.
 * 
 * A typical usage is that of a large file on disk with the leaf nodes
 * representing hashes (sha1?) of the data blocks or the data blocks themselves.
 * A quick comparison of 2 merkle trees representing 2 different files can help
 * diff the files even if they are arbitrarily large. Also, with a carefully
...
public final class MerkleTree {
    ...
    public String printTree() {
        final StringBuilder builder = new StringBuilder("printing tree...\n");
    ...
    /**
     * Represents a node in the MerkleTree data structure. Leaf nodes are
     * immutable after creation. Non-leaf nodes need to compute their hashes via
    ...
    public static final class MerkleTreeByteArrayNode implements MerkleTreeNode<byte[]> {
        ...
        @Override
        public synchronized void bloomNode(final HashingScheme hashingScheme, final boolean alreadyHashed) {
    ...
    /**
     * Curate all non-leaf nodes but recursively. It'll be nice to have an
     * iterative implementation of this behavior.
     */
    private MerkleTreeNode curateNonLeaves(final HashingScheme hashingScheme, final List<MerkleTreeNode> leveledNodes) {
    ...
    public static final class MerkleTreeByteArrayHashedSource implements MerkleTreeSource<byte[]> {
        ...
        public Iterator<byte[]> stream() {
             ...
             * 
             * @Override protected byte[] computeNext() { Iterator<byte[]>
             * hashesIter = hashes.iterator(); if (!hashesIter.hasNext()) {
             * return endOfData(); } while (hashesIter.hasNext()) { byte[] hash
             * = hashesIter.next(); if (hash != null && hash.length > 0) {
             * return hash; } } return endOfData(); } };
             */
        }
    ...
    public static final class MerkleTreeFileHashedSource implements MerkleTreeSource<byte[]> {
        ...
        // fraught with danger.
        public MerkleTreeFileHashedSource(final String fileName, int fileSplitBytes, final HashingScheme scheme)
                throws IOException {
    ...
    /**
     * Simple stand-alone utility class that provides support for various hash
     * functions.
     */
    public static final class Hasher {
        ...
        public static byte[] hash(final HashingScheme scheme, final InputStream stream) throws IOException {
            byte[] sha1 = null;
            switch (scheme) {
            case SHA1:
        ...
        // concatenated hashes
        public static byte[] computeHash(final HashingScheme hashingScheme, final List<MerkleTreeNode> children) {
            byte[] superHash = null;
            if (children.size() == 1) {
